...
class SE(nn.Module):
    ...
    def forward(self, x):
        ...
        # 激励 (Excitation)
        channel_att = self.fc(out).view(B, C, 1, 1)
        # 特征重标定
...
# 带SE的ResNet Bottleneck (仅修改注意力名称)
...
# 带SE的ResNet50特征提取器 (仅修改变量名)
class CAResNet50(nn.Module):
    def __init__(self, pretrained=True, use_se=True):
        super().__init__()
        self.inplanes = 64
        self.use_se = use_se
        ...
        if pretrained:
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'se' not in k}
...
class CADRSAN(nn.Module):
    def __init__(self, num_classes=10, pretrained=True, use_se=True):
        super().__init__()
        self.feature_layers = CAResNet50(pretrained=pretrained, use_se=use_se)
        feature_dim = 2048
