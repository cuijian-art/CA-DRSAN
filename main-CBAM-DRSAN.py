...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        out = x * spatial_att
        # 返回格式和原SE/CA完全一致，保证兼容
        return out, channel_att.squeeze(-1).squeeze(-1)


# 带CBAM的ResNet Bottleneck
...
class CAResNet50(nn.Module):
    def __init__(self, pretrained=True, use_cbam=True):
        ...
        if pretrained:
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and 'cbam' not in k}
            model_dict.update(pretrained_dict)
            self.load_state_dict(model_dict)

    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        # 🔥 修复：self.expansion → Bottleneck.expansion
        ...
        return nn.Sequential(*layers)  # 额外修复：ModuleList → Sequential 兼容ResNet结构
...
def guassian_kernel(source, target, kernel_mul=2.0, kernel_num=5, fix_sigma=None):
    n_samples = int(source.size()[0]) + int(source.size()[0])
...
class CADRSAN(nn.Module):
    def __init__(self, num_classes=10, pretrained=True, use_cbam=True):
        super().__init__()
        self.feature_layers = CAResNet50(pretrained=pretrained, use_cbam=use_cbam)
        feature_dim = 2048
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        if t_correct > correct:
            correct = t_correct
            stop = 0
            torch.save(model, 'cbam_drsan.pth')
